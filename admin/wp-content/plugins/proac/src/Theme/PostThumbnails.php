	...
	class PostThumbnails {
		...
		public function register($args = array()) {
			...
			if (null === $this->label || null === $this->id) {
				if (WP_DEBUG) {
					trigger_error(sprintf(__("The 'label' and 'id' values of the 'args' parameter of '%s::%s()' are required", 'proac'), __CLASS__, __FUNCTION__));
				}
				return;
			}
		...
		public function add_metabox() {
			...
			add_meta_box("{$this->post_type}-{$this->id}", __($this->label, 'proac'), array($this, 'thumbnail_meta_box'), $this->post_type, $this->context, $this->priority);
		}
		...
		public function add_attachment_field($form_fields, $post) {
			...
			$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail" href="#" onclick="MultiPostThumbnails.setAsThumbnail(\'%2$s\', \'%1$s\', \'%4$s\', \'%5$s\');return false;">' . __( 'Set as %3$s', 'proac' ) . '</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			// TODO:
			wp_enqueue_media( array( 'posts' => ( $post_ID ? $post_ID : null ) ) );
			wp_enqueue_script( "mpt-featured-image", $this->local_url( 'js/admin/multi-post-thumbnails-admin.js' ), array( 'jquery', 'set-post-thumbnail' ) );
			wp_enqueue_script( "mpt-featured-image-modal", $this->local_url( 'js/admin/media-modal.js' ), array( 'jquery', 'media-models' ) );				
		
			
			wp_enqueue_style( "mpt-admin-css", $this->local_url( 'css/admin/multi-post-thumbnails-admin.css' ) );
		 ...
		 */
		private function local_url($relative_path) {
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			$set_thumbnail_link = sprintf( $format_string, sprintf( esc_attr__( "Set %s" , 'proac' ), $this->label ), esc_url($image_library_url), $this->post_type, $this->id, $url_class, $this->label, $thumbnail_id );
			$content = sprintf( $set_thumbnail_link, sprintf( esc_html__( "Set %s", 'proac' ), $this->label ) );
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					...
					$content .= sprintf( $format_string, $this->post_type, $this->id, sprintf( esc_html__( "Remove %s", 'proac' ), $this->label ), $ajax_nonce );
				}
